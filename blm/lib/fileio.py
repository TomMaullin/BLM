...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# This file contains all miscellaneous file i/o functions used by BLM. These
...
#
# ============================================================================
def addBlocksToNiftis(fnames, blocks, blockIndexes, dims=None, volInds=None, affs=None, hdrs=None):

    # Get number of blocks we're adding
    nBlocks = len(fnames)

    # Create an array to tell us which blocks have been added
    blocksAdded = np.zeros(nBlocks)

    # Boolean to track if we've added all the NIFTIs to all the blocks
    allAdded = False

    # Loop through all blocks until they're all added≈
    while not allAdded:
        ...
        for i in np.arange(nBlocks):
            ...
            if not blocksAdded[i]:
                ...
                try:
                    # Create lock file, so other jobs know we are writing to this file
                    f = os.open(fname + ".lock", os.O_CREAT |
                                os.O_EXCL | os.O_RDWR)
                    fileLocked = False
                except FileExistsError:
                    fileLocked = True
                ...
                if not fileLocked:
                    ...
                    if os.path.isfile(fname):
                        ...
                        if dim is not None:
                            ...
                            # Throw an error because we don't know what to do
                            raise Exception(
                                'NIFTI does not exist and dimensions not given')
                    ...
                    for j in np.arange(nBlocks):
                        ...
                        # Check if we are looking at the same file
                        if fnames[j] == fnames[i]:

                            # If it's going to the same file, add this block
                            block = blocks[j]
                            blockInds = blockIndexes[j]
                            volInd = volInds[j]
                            ...
                            if volInd is None:
                                ...
                                # Add block
                                data[blockInds, :] = block.reshape(
                                    data[blockInds, :].shape)
                            ...
                            # Add the one volume in the correct place
                                ...
                                # # We're only looking at this volume
                                # data_vol = data[:,].reshape((n_vox,1))

                                # Add block
                                data[blockInds, volInd] = block.reshape(
                                    data[blockInds, volInd].shape)

                                # Put in the volume
                                data_out[:, :, :, volInd] = data[:, volInd].reshape(int(dim[0]),
                                                                                    int(dim[1]),
                                                                                    int(dim[2]))
                    ...
                    # Check if we've added all the blocks
                    allAdded = np.all(blocksAdded)
