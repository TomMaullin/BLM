...
def output_results(*args):
    ...
    uniq = loadFile(os.path.join(
        OutDir, "tmp", "blm_vox_uniqueM.nii")).get_fdata()
    ...
    for cv in range(nvg):
        ...
        # Ring X'Y, Y'Y
        XtY_r = readLinesFromNPY(os.path.join(
            OutDir, "tmp", 'XtY.npy'), R_inds_am).reshape([v_r, p, 1])
        YtY_r = readLinesFromNPY(os.path.join(
            OutDir, "tmp", 'YtY.npy'), R_inds_am).reshape([v_r, 1, 1])

        # Inner X'Y, Y'Y
        XtY_i = readLinesFromNPY(os.path.join(
            OutDir, "tmp", 'XtY.npy'), I_inds_am).reshape([v_i, p, 1])
        YtY_i = readLinesFromNPY(os.path.join(
            OutDir, "tmp", 'YtY.npy'), I_inds_am).reshape([v_i, 1, 1])

        if v_r:

            # Ring X'X
            XtX_r = readUniqueAtB('XtX', OutDir, R_inds,
                                  True, uniq).reshape([v_r, p, p])
            ...
            if v_lowrank:
                ...
                addBlockToNifti(os.path.join(OutDir, 'blm_vox_mask.nii'), np.zeros(
                    v_lowrank), R_inds[lowrank_inds], volInd=0, dim=NIFTIsize, aff=nifti.affine, hdr=nifti.header)
                addBlockToNifti(os.path.join(OutDir, 'blm_vox_edf.nii'), np.zeros(
                    v_lowrank), R_inds[lowrank_inds], volInd=0, dim=NIFTIsize, aff=nifti.affine, hdr=nifti.header)
                addBlockToNifti(os.path.join(OutDir, 'blm_vox_n.nii'), np.zeros(
                    v_lowrank), R_inds[lowrank_inds], volInd=0, dim=NIFTIsize, aff=nifti.affine, hdr=nifti.header)
        ...
        if v_i:
            ...
            # Inner X'X
            XtX_i = readUniqueAtB('XtX', OutDir, I_inds,
                                  False, uniq).reshape([1, p, p])

            # Check the design is full rank
            if np.linalg.matrix_rank(XtX_i) < p:
                raise Exception(
                    'The design matrix, X, is of insufficient rank.')
        ...
        if v_r:
            ...
            firstterm = -0.5*(n_sv_r.reshape(sigma2_r.shape)
                              * np.log(sigma2_r)).reshape(ete_r.shape)

            # Work out -N/2 ln(2pi)
            secondterm = -0.5*(n_sv_r.reshape(sigma2_r.shape)
                               * np.log(2*np.pi)).reshape(ete_r.shape)
            ...
            for i in range(0, p):
                for j in range(0, p):
                    ...
                    if v_r:
                        ...
                        # Output
                        addBlockToNifti(os.path.join(OutDir, 'blm_vox_cov.nii'), covbetaij_r,
                                        R_inds, volInd=vol, dim=dimCov, aff=nifti.affine, hdr=nifti.header)
                    ...
                    if v_i:
                        ...
                        # Output
                        addBlockToNifti(os.path.join(OutDir, 'blm_vox_cov.nii'), covbetaij_i,
                                        I_inds, volInd=vol, dim=dimCov, aff=nifti.affine, hdr=nifti.header)
        ...
        for i in range(0, c):
            ...
            if statType == 'T':
                ...
                if v_r:
                    ...
                    pc_r[tStatc_r < 0] = - \
                        np.log10(
                            1-stats.t.cdf(tStatc_r[tStatc_r < 0], df_r[tStatc_r < 0]))
                    pc_r[tStatc_r >= 0] = - \
                        np.log10(
                            stats.t.cdf(-tStatc_r[tStatc_r >= 0], df_r[tStatc_r >= 0]))

                    # Remove infs
                    if "minlog" in inputs:
                        pc_r[np.logical_and(
                            np.isinf(pc_r), pc_r < 0)] = inputs['minlog']
                    else:
                        pc_r[np.logical_and(
                            np.isinf(pc_r), pc_r < 0)] = -323.3062153431158
                ...
                if v_i:
                    ...
                    pc_i[tStatc_i < 0] = - \
                        np.log10(1-stats.t.cdf(tStatc_i[tStatc_i < 0], df_i))
                    pc_i[tStatc_i >= 0] = - \
                        np.log10(stats.t.cdf(-tStatc_i[tStatc_i >= 0], df_i))

                    # Remove infs
                    if "minlog" in inputs:
                        pc_i[np.logical_and(
                            np.isinf(pc_i), pc_i < 0)] = inputs['minlog']
                    else:
                        pc_i[np.logical_and(
                            np.isinf(pc_i), pc_i < 0)] = -323.3062153431158
            ...
            if statType == 'F':
                ...
                if v_r:
                    ...
                    iLvectiXtXLvec_r = np.linalg.pinv(
                        LvectiXtXLvec_r).reshape([v_r, q*q])

                    # Calculate the numerator of the F statistic for the ring
                    Fnumerator_r = np.matmul(
                        Lbetat_r, np.linalg.solve(LvectiXtXLvec_r, Lbeta_r))
                ...
                if v_i:
                    ...
                    iLvectiXtXLvec_i = np.linalg.pinv(
                        LvectiXtXLvec_i).reshape([1, q*q])

                    # Calculate the numerator of the F statistic for the ring
                    Fnumerator_i = np.matmul(
                        Lbetat_i, np.linalg.solve(LvectiXtXLvec_i, Lbeta_i))
    ...
    if blocks:
        addBlocksToNiftis(fnames, blocks, blockIndexes,
                          dims, volInds, affs, hdrs)
...
def readAndSumUniqueAtB(AtBstr, OutDir, vinds, n_b, sv, jobNum):
    ...
    for batchNo in range(2, (n_b+1)):
        ...
        for m in range(1, maxM+1):
            ...
            if sv:
                AtB_batch[np.where(uniquenessMask == m),
                          :] = AtB_batch_unique[(m-1), :]
            else:
                # Work out X'X for the inner
                if uniquenessMask == m:
...
def readUniqueAtB(AtBstr, OutDir, vinds, sv, uniquenessMask):
    ...
    AtB_unique = readLinesFromNPY(os.path.join(
        OutDir, "tmp", AtBstr+".npy"), uniqueVals)
