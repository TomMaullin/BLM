...
def compute_product_forms(*args):
    ...
    memorySafeAtB(X.reshape(1, X.shape[0], X.shape[1]), Y, MAXMEM, os.path.join(
        OutDir, "tmp", "XtY.npy"))
...
def memorySafeAtB(A, B, MAXMEM, filename):
    ...
    if not os.path.isfile(filename):
        ...
        M = open_memmap(filename, mode='w+',
                        dtype='float64', shape=(v, pORone))
        ...
        voxelGroups = np.array_split(
            np.arange(v, dtype='int32'), v//vPerBlock+1)

        # Loop through each group of voxels saving A'B for those voxels
        for vb in range(int(v//vPerBlock+1)):
            M[voxelGroups[vb], :] = (A.transpose(
                0, 2, 1) @ B[voxelGroups[vb], :, :]).reshape(len(voxelGroups[vb]), pORone)
    ...
    # Otherwise we add to the memory map that does exist
        ...
        # Loop through each group of voxels saving A'B for those voxels
        for vb in range(int(v//vPerBlock+1)):
            M[voxelGroups[vb], :] = M[voxelGroups[vb], :] + \
                (A.transpose(0, 2, 1) @
                 B[voxelGroups[vb], :, :]).reshape(len(voxelGroups[vb]), pORone)
