...
fsl_sub -l log/ -N setup bash ./scripts/cluster_blm_setup.sh $inputs > /tmp/$$ && setupID=$(awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' /tmp/$$)
if [ "$setupID" == "" ] ; then
...
while [ "$i" -le "$nb" ]; do
  ...
  fsl_sub -j $setupID -l log/ -N batch${i} bash ./scripts/cluster_blm_batch.sh $i $inputs > /tmp/$$ && batchIDs=$(awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' /tmp/$$),$batchIDs
...
# Submit results job 
fsl_sub -j $batchIDs -l log/ -N results bash ./scripts/cluster_blm_concat.sh $inputs > /tmp/$$ && resultsID=$(awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' /tmp/$$)
if [ "$resultsID" == "" ] ; then
